...
import io.kaitai.struct.translators.{QtCppTranslator, TypeDetector}
...
class QtCppCompiler(
  ...
  override val translator = new QtCppTranslator(typeProvider, importListSrc, importListHdr, config)
  ...
  override def fileHeader(topClassName: String): Unit = {
    ...
    importListHdr.addKaitai("kaitai-qt/kaitaistruct.h")
    importListHdr.addSystem("stdint.h")
    importListHdr.addSystem("QObject")
    ...
    outHdr.puts(
      "#error \"Incompatible Kaitai Struct Qt API: version " +
  ...
  override def classHeader(name: List[String]): Unit = {
    ...
    val parentsPart = name.dropRight(1)
    val parents = if (parentsPart.nonEmpty) types2class(parentsPart) else ""
    val separator = if (parents.nonEmpty) "::" else ""

    val className = types2class(List(name.last))

    outHdr.puts
    outHdr.puts(s"class $parents$separator$className : public $kstructName {")
    outHdr.inc
    accessMode = PrivateAccess
    outHdr.puts("Q_OBJECT")
  ...
  override def runReadCalc(): Unit = {
    ...
    outSrc.inc
    importListSrc.addKaitai("kaitai-qt/exceptions.h")
  ...
  override def attributeDeclaration(attrName: Identifier, attrType: DataType, isNullable: Boolean): Unit = {
    ...
    outHdr.puts(s"Q_PROPERTY(${kaitaiType2NativeType(attrType)} ${idToStr(attrName)} MEMBER ${privateMemberName(attrName)})")
  ...
  override def attrInit(attr: AttrLikeSpec): Unit = {
    attr.dataTypeComposite match {
      case _: UserType | _: ArrayTypeInStream | OwnedKaitaiStreamType =>
        // data type will be pointer to user type, QList or stream, so we need to init it
  ...
  /**
    * Generates QList contents destruction loop.
    * @param elType element type, i.e. XXX in `QList&lt;XXX&gt;`
    * @param arrVar variable name that holds pointer to QList
    */
  def destructVector(elType: String, arrVar: String): Unit = {
    outSrc.puts(s"for (QList<$elType>::iterator it = $arrVar->begin(); it != $arrVar->end(); ++it) {")
    outSrc.inc
  ...
  def newVector(elType: DataType): String = {
    ...
    config.cppConfig.pointers match {
      case RawPointers =>
        s"new QList<$cppElType>()"
      case UniqueAndRawPointers =>
        s"std::unique_ptr<QList<$cppElType>>(new QList<$cppElType>())"
  ...
  override def idToStr(id: Identifier): String = QtCppCompiler.idToStr(id)

  override def publicMemberName(id: Identifier): String = idToStr(id)

  override def privateMemberName(id: Identifier): String = QtCppCompiler.privateMemberName(id)
  ...
  override def type2class(className: String): String = QtCppCompiler.type2class(className)

  def kaitaiType2NativeType(attrType: DataType, absolute: Boolean = false): String =
    QtCppCompiler.kaitaiType2NativeType(config.cppConfig, importListHdr, attrType, absolute)
...
object QtCppCompiler extends LanguageCompilerStatic
  with StreamStructNames {
  override def getCompiler(
    tp: ClassTypeProvider,
    config: RuntimeConfig
  ): LanguageCompiler = new QtCppCompiler(tp, config)
  ...
  def kaitaiType2NativeType(config: CppRuntimeConfig, importListHdr: CppImportList, attrType: DataType, absolute: Boolean = false): String = {
    attrType match {
      ...
      case at: ArrayType => {
        importListHdr.addSystem("QList")
        val vecType = s"QList<${kaitaiType2NativeType(config, importListHdr, at.elType, absolute)}>"
