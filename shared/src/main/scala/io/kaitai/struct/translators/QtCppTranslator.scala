...
class QtCppTranslator(provider: TypeProvider, importListSrc: CppImportList, importListHdr: CppImportList, config: RuntimeConfig)
  ...
  override def doArrayLiteral(t: DataType, values: Seq[expr]): String = {
    if (config.cppConfig.useListInitializers) {
      ...
      val cppElType = QtCppCompiler.kaitaiType2NativeType(config.cppConfig, importListHdr, t)
  ...
  override def genericBinOp(left: Ast.expr, op: Ast.operator, right: Ast.expr, extPrec: Int) = {
    (detectType(left), detectType(right), op) match {
      case (_: IntType, _: IntType, Ast.operator.Mod) =>
        s"${QtCppCompiler.kstreamName}::mod(${translate(left)}, ${translate(right)})"
      case _ =>
  ...
  override def doInternalName(id: Identifier): String =
    QtCppCompiler.privateMemberName(id)

  override def doEnumByLabel(enumSpec: EnumSpec, label: String): String = {
    val isExternal = enumSpec.isExternal(provider.nowClass)
    if (isExternal) {
      importListHdr.addLocal(QtCppCompiler.outFileNameHeader(enumSpec.name.head))
    }
    QtCppCompiler.types2class(enumSpec.name.dropRight(1)) + "::" +
      Utils.upperUnderscoreCase(enumSpec.name.last + "_" + label)
  }
  override def doEnumById(enumSpec: EnumSpec, id: String): String =
    s"static_cast<${QtCppCompiler.types2class(enumSpec.name)}>($id)"
  ...
  override def doCast(value: Ast.expr, typeName: DataType): String =
    s"static_cast<${QtCppCompiler.kaitaiType2NativeType(config.cppConfig, importListHdr, typeName)}>(${translate(value)})"

  // Predefined methods of various types
  override def strToInt(s: expr, base: expr): String = {
    val baseStr = translate(base)

    s"${QtCppCompiler.kstreamName}::string_to_int(${translate(s)}" + (baseStr match {
      case "10" => ""
      case _ => s", $baseStr"
    }) + ")"
  }
  ...
  override def intToStr(i: expr): String =
    ...
    s"${QtCppCompiler.kstreamName}::to_string(${translate(i)})"
  override def bytesToStr(bytesExpr: String, encoding: String): String =
    s"""${QtCppCompiler.kstreamName}::bytes_to_str($bytesExpr, ${doRawStringLiteral(encoding)})"""
  ...
  override def bytesMin(b: Ast.expr): String =
    s"${QtCppCompiler.kstreamName}::byte_array_min(${translate(b)})"
  override def bytesMax(b: Ast.expr): String =
    s"${QtCppCompiler.kstreamName}::byte_array_max(${translate(b)})"

  override def strLength(s: expr): String =
    s"${translate(s, METHOD_PRECEDENCE)}.length()"
  override def strReverse(s: expr): String =
    s"${QtCppCompiler.kstreamName}::reverse(${translate(s)})"
